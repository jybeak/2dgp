...
class Boss_monster:
    ...
    def __init__(self):
        ...
        self.life = 20
        if Red_plant_monster.red_plant_monster_image is None:
            Red_plant_monster.red_plant_monster_image = load_image('resouce/image/boss_monster_image.png')
        if Red_plant_monster.monster_death_sound == None:
            Red_plant_monster.monster_death_sound = load_wav('???')
            Red_plant_monster.monster_death_sound.set_volume(32)

    def update(self, frame_time):
        global boss_monster_time
        if main_state.ui.score >=300:
            boss_monster_time += frame_time
            distance = Red_plant_monster.RUN_SPEED_PPS * frame_time
            self.x -= distance
            if self.x < 600 and boss_monster_time > 4:
                pass
